...
{
    Student tab[MAX_E]; // tab[20]d'enregistrement logique (student)
    int NB;             // nombre d'enregistrement logique (student) dans chaque bloc
    int svt;
} block, *blockP;


void printBlock(blockP S); // afficher un block

void createBlock(blockP S); // lire les informations d'un block
...
void creatstudent(LOF_fileP f, char file_name[20], int matricule);//la cration d'un student
